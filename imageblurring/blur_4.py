...
from imageblurring.imagefunctions import readImageFromFile, \
    writeImageToFile, padImage
from imageblurring.cython_blur import unvectorizedBlurCython, \
    vectorizedBlurCython


def main(inimg="beatles.jpg",
         outimg="blurred_images\\blur_cython.jpg",
         blurCycles=1,
         vectorized=False):
    """The function is using a Cython to blur a picture. It takes
    in 4 arguments, reads a picture from file and blurs the image
    using the either the vectorized or none vectorized blur
    algorithm, and writes the image to a file. The function also
    tracks runtime for the algorithm that blurs the image.
    ...
    for i in range(blurCycles):
        if (vectorized):
            image = vectorizedBlurCython(padImage(image))
        else:
            image = unvectorizedBlurCython(padImage(image))

    vectOrNot = "vectorized" if vectorized else "unvectorized"
    print("Runtime Cython", vectOrNot, "blur:",
...
if __name__ == '__main__':
    if (len(sys.argv) > 4):
        ...
        winFilename = "blurred_images\\blur_cython.jpg"
        linuxFilename = "blurred_images/blur_cython.jpg"
